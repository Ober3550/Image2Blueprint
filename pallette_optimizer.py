...
for p_index in range(len(pallette)):
    bp.addEntity(
        item_name[p_index],
        (math.floor(p_index / row_length), p_index % row_length),
        item_type[p_index],
    )

minimum_difference = 255
while len(pallette) > desired_size:
        ...
        for b in pallette_iterb:
            ...
            if difference < minimum_difference:
                try:
                    ...
                    pass
    print("Pallette Size: ", len(pallette))
    minimum_difference = minimum_difference + 10

bp_string = open("pallettes/alien_biomes_optimized.txt", "w+")
bp_string.write(
    "".join(
        [
            str(pallette[i]) + ":" + item_name[i] + ":" + item_type[i] + "\n"
